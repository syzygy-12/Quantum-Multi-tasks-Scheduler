...
def generate_small_circuits(num_circuits=30, max_qubits=4):
    """生成小于5量子比特的量子电路"""
    circuits = []
    
    for i in range(num_circuits):
        # 随机选择量子比特数量 (2-4)
        n_qubits = random.randint(2, max_qubits)
        ...
        for _ in range(depth):
            # 随机选择门类型
            if n_qubits > 1 and random.random() < 0.3:  # 30%概率添加双量子比特门
        ...
        # 添加到电路列表
        circuits.append((f"small_circuit_{i+1}_{n_qubits}q", qc))
...
def main_small_circuits():
    """主函数：生成小于5量子比特的电路"""
    print("生成小于5量子比特的量子电路...")
    
    # 生成电路
    circuits = generate_small_circuits(30, 4)
    
    # 保存为QPY文件
    output_dir = "small_qubit_circuits"
    saved_files = save_circuits_to_qpy(circuits, output_dir)
    
    # 输出摘要
    print(f"\n生成完成! 共创建了 {len(saved_files)} 个小于5量子比特的量子电路")
