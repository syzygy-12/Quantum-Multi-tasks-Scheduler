...
def generate_large_circuits(num_circuits=30, min_qubits=11, max_qubits=14):
    """生成大于10但小于15量子比特的量子电路"""
    circuits = []
    ...
    for i in range(num_circuits):
        ...
        for _ in range(depth):
            # 随机选择门类型
            if random.random() < 0.5:  # 50%概率添加双量子比特门
        ...
        # 添加到电路列表
        circuits.append((f"large_circuit_{i+1}_{n_qubits}q", qc))
...
def main_large_circuits():
    """主函数：生成大于10但小于15量子比特的电路"""
    print("生成大于10但小于15量子比特的量子电路...")
    
    # 生成电路
    circuits = generate_large_circuits(30, 11, 14)
    
    # 保存为QPY文件
    output_dir = "large_qubit_circuits"
    saved_files = save_circuits_to_qpy(circuits, output_dir)
    
    # 输出摘要
    print(f"\n生成完成! 共创建了 {len(saved_files)} 个大于10但小于15量子比特的量子电路")
